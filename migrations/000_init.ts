...
export async function up(sql: Sql) {
  ...
  await sql`
    create index idx_messages_from_to_user_id on messages (from_user_id, to_user_id);
  `.exec();
  ...
  for (let i = 0; i < 250; i++) {
    ...
    await sql`
      with
        new_message as (
          insert into
            messages (body, from_user_id, to_user_id, created_at)
          values
            (
              ${body},
              ${fromUserId},
              ${toUserId},
              now() - '1 hour'::interval * random() * 10000
            )
          returning
            id
        ),
    ...
    set
      last_message_id = (
        ...
        from
          messages
        where
          messages.from_user_id in (threads.from_user_id, threads.to_user_id)
